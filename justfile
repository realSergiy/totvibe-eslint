...
upgrade *args='':
    pnpm run --silent toolchain-report -- --update
...
# Interactively select toolchain and JS upgrades, then non-interactively upgrade Python (uv has no interactive mode) and reinstall both.
upgrade-interactive:
    @pnpm run --silent toolchain-report
