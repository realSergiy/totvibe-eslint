...
def test_6_1_1_skips_repos_with_no_pyproject_file(run_pyrefly: RunPyrefly, skip: MakeFinding) -> None:
...
def test_6_1_2_skips_repos_with_a_pyproject_file_but_no_python_source(
    run_pyrefly: RunPyrefly, skip: MakeFinding
) -> None:
...
def test_6_2_2_fails_when_pyrefly_config_lives_in_pyproject_instead(run_pyrefly: RunPyrefly, fail: MakeFinding) -> None:
    result = run_pyrefly(pyproject='[tool.pyrefly]\npreset = "strict"\n')

    assert result.findings == [fail("pyrefly config lives in pyproject.toml; move it to a standalone pyrefly.toml")]


def test_6_2_3_errors_when_pyrefly_config_cannot_be_parsed(run_pyrefly: RunPyrefly, error: MakeFinding) -> None:
    result = run_pyrefly(pyrefly="preset = [unterminated\n")

    assert result.findings == [error("could not parse pyrefly.toml")]


def test_6_3_1_fails_when_preset_is_not_strict(run_pyrefly: RunPyrefly, fail: MakeFinding) -> None:
...
def test_6_5_1_fails_when_top_level_errors_weaken_strict_for_all_code(
    ...
    assert result.findings == [fail("top-level errors table weakens strict on all code: implicit-any")]
...
def test_6_5_2_fails_when_an_error_kind_is_set_stray_at_the_top_level(
    ...
    assert result.findings == [fail("pyrefly.toml sets error kinds at the top level, not under errors: implicit-any")]
...
def test_6_6_1_fails_when_a_sub_config_weakens_strict_for_production_or_test_code(
    run_pyrefly: RunPyrefly, glob: str, fail: MakeFinding
) -> None:
    pyrefly = _PYREFLY_TESTS_RELAXED.replace('"tests/cerberus/**"', f'"{glob}"')

    result = run_pyrefly(pyrefly=pyrefly)

    assert result.findings == [fail(f"sub-config `{glob}` weakens strict; no relaxations allowed: implicit-any")]


def test_6_6_2_fails_when_a_sub_config_entry_is_not_a_table(run_pyrefly: RunPyrefly, fail: MakeFinding) -> None:
    pyrefly = _PYREFLY_STRICT + '\nsub-config = ["oops"]\n'

    result = run_pyrefly(pyrefly=pyrefly)

    assert result.findings == [fail("pyrefly.toml sub-config entries must be tables; found 'oops'")]


def test_6_7_1_passes_when_preset_is_strict_coverage_is_complete_and_relaxations_are_absent(
    run_pyrefly: RunPyrefly, ok: MakeFinding
) -> None:
    result = run_pyrefly()
